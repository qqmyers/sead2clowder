...
class Tags @Inject()(collections: CollectionService, datasets: DatasetService, files: FileService, sections: SectionService) extends SecuredController {
  ...
  def createTagList(user: Option[User]) = {
    ...
    current.plugin[ElasticsearchPlugin] match {
      case Some(plugin) => {
        val results = plugin.listTags("data", Array[String]("tag"), None)
        for ((k,v) <- results) {
...
//    }
    ...
    current.plugin[ElasticsearchPlugin] match {
      case Some(plugin) => {
        for ((k,v) <- plugin.listTags("data", Array[String]("tag"), Some("dataset"))) {
          weightedTags(k) = weightedTags(k) + v * current.configuration.getInt("tags.weight.dataset").getOrElse(1)
        }
        for ((k,v) <- plugin.listTags("data", Array[String]("tag"), Some("file"))) {
    ...
    // TODO: Should we index sections in ElasticSearch, remove this Mongo query, or something else?
