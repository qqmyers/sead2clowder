...
class Files @Inject() (
  ...
  def list(when: String, date: String, limit: Int, listView: String) = SecuredAction(authorization = WithPermission(Permission.ListFiles)) { implicit request =>
    ...
    Ok(views.html.filesList(fileList, prev, next, limit, listView, rightsForUser))
  }
	        ...
	        file match {
              ...
              if(fileType.equals("application/xml") || fileType.equals("text/xml")){
            	  val xmlToJSON = FilesUtils.readXMLgetJSON(uploadedFile.ref.file)
            		  ...
            		  _.index("data", "file", id, List(("filename",f.filename), ("contentType", f.contentType),("datasetId",""),("datasetName",""), ("xmlmetadata", xmlToJSON), ("author", identity.fullName), ("uploadDate", dateFormat.format(new Date()))  ))
            	  }
              }
              else{
            	  current.plugin[ElasticsearchPlugin].foreach{
            		  _.index("data", "file", id, List(("filename",f.filename), ("contentType", f.contentType),("datasetId",""),("datasetName",""), ("author", identity.fullName), ("uploadDate", dateFormat.format(new Date())) ))
            	  }
              }
...
user match {
  case Some(identity) => {
    request.body.file("File").map { f =>
      ...
      //Upload from user's machine
             
      //Can't have both an uploaded file from user's machine and a file from a URL
...
<<<<<<< HEAD
        ...
        }case false => Redirect(routes.Files.uploadFile()).flashing("error"->"Please select ONE file (upload from your computer or from URL)")
=======
	        }case false => Redirect(routes.Files.uploadFile()).flashing("error"->"Please select ONE file (upload from your computer or from URL)")
	       }
	      }.getOrElse {
	         ...
	         try{	
		              ...
		              file match {
		                case Some(f) => {
		                  ...
		                  if (fileType.equals("application/xml") || fileType.equals("text/xml")) {
		                    ...
		                    if(toBeCurated)
			                    current.plugin[ElasticsearchPlugin].foreach {
			                      _.index("data", "file", id, List(("filename", filename), ("contentType", f.contentType), ("author", identity.fullName), ("uploadDate", dateFormat.format(new Date())),("xmlmetadata", xmlToJSON)))
			                    }
			              ...
			              if(toBeCurated)
				            current.plugin[ElasticsearchPlugin].foreach{
				              _.index("data", "file", id, List(("filename",filename), ("contentType", f.contentType), ("author", identity.fullName), ("uploadDate", dateFormat.format(new Date()))))
				            }
			            }
	           ...
	           case ex: MalformedURLException =>{
	             Redirect(routes.Files.uploadFile()).flashing("error"->"URL was invalid or no URL entered")
	           }
	         }
	
	      }
>>>>>>> refs/remotes/origin/feature/MMDB-1582-Adaptation-of-new-Extractions-stuff-for-regular-file-upload-and-curation
      }
         ...
         try{	
	            ...
	            try {
	              ...
	              var fileType = mimetype
	              Logger.debug("fileType= " + fileType)
	              val file = files.save(new FileInputStream(localfile), localfile.getName(), Some(ct), identity, showPreviews, isPublic)
	              val uploadedFile = localfile
  ...
  case None => Redirect(routes.Files.list()).flashing("error" -> "You are not authorized to create new files.")
 }
}
