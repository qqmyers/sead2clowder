...
class Collections @Inject()(datasets: DatasetService, collections: CollectionService, previewsService: PreviewService,
  ...
  private def unZipIt(inputStream : InputStream, tempFile : java.io.File): Unit = {
  ...
  private def deleteTempDir(currentFile : java.io.File) {
    if (currentFile.isDirectory){
      var currentContents = currentFile.listFiles()
      for (content <- currentContents){
        deleteTempDir(content)
      }
      try{
        currentFile.delete()
      } catch {
      ...
      try {
        ...
        case e : Exception => Logger.error("Could not delete file" + currentFile + " error :" +e.getMessage)
      }
    }
  }
  ...
  private def processFolder(currentFolder : java.io.File, parentId : Option[UUID], user : User, clowderurl : String,creator_url : String): Option[UUID] ={
    ...
    parentId match {
      ...
      case None => {//THERE IS NO PARENT ID
        if (hasDirectory && !hasFile) {
  ...
  private def createFromZip(fileId: UUID, user : Option[User], clowderurl : String, creator_url : String) : Option[UUID] =  {
