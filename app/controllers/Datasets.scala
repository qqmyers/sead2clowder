    ...
    parentCollectionId match {
      case Some(parent_id) => {
        collections.get(UUID(parent_id)) match {
          case Some(parentCollection) => Ok("not yet implemented")//(views.html.newDatasetWithParentCollection(decodedSpaceList.toList, decodedCollectionslist.toList, RequiredFieldsConfig.isNameRequired, RequiredFieldsConfig.isDescriptionRequired, parentCollectionId)).flashing("error" -> "Please select ONE file (upload new or existing)")
