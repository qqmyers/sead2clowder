...
class MongoDBDatasetService @Inject() (
        ...
        for( i <- 1 to (rdfDescriptions.length - 1)){
          ...
          if(rdfDescriptions(i).contains("<rdf:type")){
            ...
            if(isInRootNodes){
            			...
            			var connection = "<rdf:Description rdf:about=\"" + theHost +"/api/datasets/"+ id.stringify
  ...
  def addTags(id: UUID, userIdStr: Option[String], eid: Option[String], tags: List[String]): List[String] = {
    ...
    var tagIDsList: List[String] = List.empty
    tags.foreach(tag => {
      ...
      if (!existingTags.contains(tag)) {
        ...
        tagIDsList = tagIDsList :+ tagObj.id.stringify 
      }
    })
    return tagIDsList
  }
