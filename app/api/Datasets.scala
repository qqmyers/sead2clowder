      	      ...
      	      files.get(UUID(file_id)) match {
	              ...
	              (request.body \ "isPublic").asOpt[Boolean].map {
	                inputIsPublic=>
      	           ...
      	           val d = Dataset(name=name,description=description, created=new Date(), files=List(file), author=request.user.get, isPublic = Some(request.user.get.fullName.equals("Anonymous User")||isPublic ), licenseData = new LicenseData())
