...
class Files @Inject()(
  ...
  def addVersusMetadata(id: UUID) =
    SecuredAction(authorization = WithPermission(Permission.AddFilesMetadata), resourceId = Some(id)) { request =>
  ...
  @ApiOperation(value = "Upload file",
      ...
      responseClass = "None", httpMethod = "POST")
  def upload(showPreviews: String = "DatasetLevel", originalZipFile: String = "", fileIsPublic: String = "false", flagsFromPrevious: String = "") = SecuredAction(parse.multipartFormData, authorization = WithPermission(Permission.CreateFiles)) {
    implicit request =>
      request.user match {
        case Some(user) => {
  ...
  @ApiOperation(value = "Upload a file to a specific dataset",
      ...
      responseClass = "None", httpMethod = "POST")
  def uploadToDataset(dataset_id: UUID, showPreviews: String="DatasetLevel", originalZipFile: String = "", fileIsPublic: String = "false", flagsFromPrevious: String = "") = SecuredAction(parse.multipartFormData, authorization=WithPermission(Permission.CreateDatasets), Some(dataset_id)) { implicit request =>
    request.user match {
     case Some(user) => {
  ...
  def attachPreview(file_id: UUID, preview_id: UUID) = SecuredAction(authorization = WithPermission(Permission.CreateFiles), resourceId = Some(file_id)) {
    ...
    // Use the "extractor_id" field contained in the POST data.  Use "Other" if absent.
      ...
      val extractor_id = if (eid.isDefined) {
        ...
        Logger.debug("api.Files.attachPreview(): No \"extractor_id\" specified in request, set it to Other.  request.body: " + request.body.toString)
        Some("Other")
      }
      request.body match {
  ...
  */
  ...
  def extract(id: UUID) = SecuredAction(parse.anyContent, authorization = WithPermission(Permission.ShowFilesMetadata), resourceId = Some(id)) { implicit request =>
